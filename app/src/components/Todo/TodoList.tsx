...
const TodoList = ({ users }) => {
            ...
            <li key={idx} className="flex justify-between p-2 hover:bg-blue-300">
              ...
              <div>
                <button
                  ...
                  onClick={() => handleFinish(x)}
                >
                  完成
                </button>
                <button
                  className="bg-red-500 hover:bg-blue-700 text-white py-1 px-2 ml-1 rounded-full"
  ...
  const handleFinish = ({ id, status }: { id: number; status: boolean | number }) => {
    ...
    _request('put', '/api/todo', JSON.stringify(obj))
      .then(() => {
  ...
  const addTodo = (data: todoProps) => {
    _request('post', '/api/todo', JSON.stringify(data))
      .then(() => {
  ...
  return (
    <div className="w-1/2">
      ...
      {activeKey === 'todo' ? todoRender(list) : null}
      {activeKey === 'finished' ? fininshRender(list) : null}
    </div>
  )
}
